...
object  DWReleaseClick{
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String)={
    ...
    try {
      ...
      //读取hive表数据
      val clickReleaseDF: Dataset[Row] = SparkHelper.readTable(spark, ReleaseConstant.ODS_RELEASE_SESSION, clickColumns)
        .where(clickReleaseCondition)
        .repartition(ReleaseConstant.DEF_REPARTITION_NUMBER)

     // clickReleaseDF.show(10)
      SparkHelper.writeTableData(clickReleaseDF,ReleaseConstant.DW_RELEASE_EXPOSURE,savemode)
